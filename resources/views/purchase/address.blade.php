<x-app-layout>
    <div class="max-w-[680px] mx-auto mt-12 mb-[135px]">
        <h2 class="text-4xl font-bold text-center mb-[103px]">住所の変更</h2>
        ...
        <form action="{{ route('purchase.update.address', $item->id) }}" method="POST">
            ...
            <div>
                ...
                <input type="text" id="shipping_postal_code" class="block w-full border border-[#5F5F5F] h-[45px]
                    rounded px-4"
            ...
            <div class="mt-[68px]">
                ...
                <input type="text" id="shipping_address" class="block w-full border border-[#5F5F5F] h-[45px]
                    rounded px-4"
                ...
                <ul class="text-sm text-red-600 space-y-1 mt-2">
                    @foreach ((array) $errors->get('shipping_address') as $message)
                    <li>{{ $message }}</li>
                    @endforeach
                </ul>
                @endif
            ...
            <div class="mt-[90px]">
                ...
                <input type="text" id="shipping_building" class="block w-full border border-[#5F5F5F] h-[45px]
                    rounded px-4"
                    name="shipping_building" value="{{ old('building',
                    $purchaseAddress ? $purchaseAddress['shipping_building'] : $profile->building) }}" />
            </div>

            <button type="submit" class="w-full text-white font-bold text-[26px] bg-[#FF5555] rounded-[5px] flex items-center justify-center  mt-[111px] h-[60px]">更新する</button>
        </form>
